...
// func (t *Tabulate) SetAlignA(aligns []string) {
// 	t.AlignA = aligns
// }
func (t *Tabulate) SetWidths(widths []int) {
	t.Widths = widths
}
func (t *Tabulate) EnableRawOut(widths []int) {
	t.isRawOut = true
	t.Widths = widths
}
func (t *Tabulate) DisableRawOut() {
	t.isRawOut = false
}
...
func Create(data interface{}) *Tabulate {
	...
	case [][]Fielder:
		t.Data = createFromFielder(data.([][]Fielder))
	default:
		fmt.Println(v)
	}
