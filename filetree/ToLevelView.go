...
func (f *FileList) ToLevelView(pad string, isExtended bool) string {
	...
	for i, dir := range dirs {
		...
		if len(fm[dir]) > 1 {
			if !paw.EqualFold(dir, RootMark) {
				if f.depth != 0 {
					...
					fmt.Fprint(w, fm[dir][0].ColorWrapDirName(pipad, bannerWidth))
				}
			}
		} else {
			continue
		...
		fds.ModifyWidth(f, bannerWidth-paw.StringWidth(ppad))
